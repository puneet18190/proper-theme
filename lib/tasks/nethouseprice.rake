task :nethouseprice => :environment do
  require "open-uri"
  require 'nokogiri'
  require 'zip'
  ...
  # Thread.new do
    ...
    Zip::OutputStream.open(t.path) do |z|
      @data.each_with_index do |item,i|
        sp = "A56CC33A024AD3_SP"+item.created_at.year.to_s.split(//).last(2).join()+item.created_at.month.to_s.rjust(2,'0')+item.id.to_s.rjust(4,'0')
        (0..9).each do |n|
      ...
      d=DateTime.now
      seq = "01"
      f_name = "A56CC33A024AD3_"+d.year.to_s+d.month.to_s.rjust(2,'0')+d.day.to_s+seq
      z.put_next_entry("#{f_name}.blm")
      ac = ApplicationController.new()
      remote_data = ac.render_to_string "properties/download_blm3", :locals => {:@data => @data}, :layout=>false
    ...
    t.close
    UserMailer.blm_status("BLM Uploaded Successfully on NetHousePrice.").deliver
