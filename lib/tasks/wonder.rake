task :wonder => :environment do
  ...
  # Thread.new do
    puts "===============Upload Start========================="
    #UserMailer.blm_status("BLM Upload on WonderProperty Start").deliver
    @data = Property.where(:approval_status=>"approved", :wonder_property=>true, :portal_visibility => true)
    ...
    # end
    ftp = Net::FTP.new ('ftp.wonderproperty.com')
    ftp.passive = true
    ftp.login 'Seal-Properties', 'Sp9Z00IcdkGHJlysrIGu'
    ftp.putbinaryfile(t.path,"39545.zip")
    t.close
    #UserMailer.blm_status("BLM Uploaded Successfully on WonderProperty.").deliver
